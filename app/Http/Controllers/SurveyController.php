...
//
    ...
    public function layketquabaocao_khonghailong($orgid=9,$svid){
        $this->xuatexcel=1;
        $organization=Organization::find($orgid); 
        ...
        if(isset($_GET['filter_survey_topic_id']) && $_GET['filter_survey_topic_id']!=0){
            ...
            $survey= Survey::where('survey_id',$svid)->get();
            ...
            if(isset($_GET['xuatexcel'])){
                ...
                $sheet_header=[['BÁO CÁO KHẢO SÁT CHƯA HÀI LÒNG '.date("d-m-Y")],$arr_header_excel];
    ...
    public function tinhdeshowsv($survey_id){
        ...
        //==========
        $data=array('survey'=>$survey,'data'=>$data,'topic'=>$topic,'question'=>$question,'answer'=>$ans,'result'=>$result);

        return $data;
    }

    public function guiemaildanhgiakhonghailong($svid,$idorglv1){

        $orglv1=Organization::where('org_id',$idorglv1)->get();

        foreach($orglv1 as $no1=>$val){
            if($val->org_nhanemail!="" && $val->org_nhanemail!=" "){
                $users_nhanemail=explode(";",$val->org_nhanemail);
                //print_r($users_nhanemail);
                $dt=$this->tinhdeshowsv($svid);
                //$bc1=$this->layketquabaocao_khonghailong($val->org_id,$svid);
                //$bc2=$this->layketquabaocaongay_bc2($val->org_id);
                //$bc3=$this->layketquabaocaongay_bc3($val->org_id);
 
                //--------
                //echo 'nhanmail: '.$val->org_nhanemail.'<br>';
                foreach($users_nhanemail as $no2=>$val2){
                    if (filter_var(trim($val2), FILTER_VALIDATE_EMAIL)) {
                        //echo $no.'---'.$val->org_name.'--.'.$val2.'<br>';
                        //$data['to_email']=trim($val2);
                        $data=array();
                        $data[$no1][$no2] = array(
                          'survey'=>$dt['survey'],'data'=>$dt['data'],'topic'=>$dt['topic'],'question'=>$dt['question'],'answer'=>$dt['answer'],'result'=>$dt['result'],
                          ...
                          'view_blade'=>'admin.survey.baocao_khonghaiong',
                          'bc1'=> '',//$bc1,
                          'bc2'=> '',
                          'bc3'=> '',//storage_path('app/public/').'fileName1.xlsx',
                        );

                        //echo '<br>========::'.($no2+1).'<br>';
                       // print_r($data[$no1][$no2]['to_email']);
                        //echo '<br>========';
                        Guithu::sendmail($data[$no1][$no2]);
                    }
                }
            }
        }
    ...
    public function baocao1(){
      /*Lặp:
        ...
        foreach($orglv1 as $no1=>$val){
            //echo '===========================================================<br>Tổ chuc: '.$val->org_name.'<br>';
    ...
    public function listsurvey(){
        ...
        if($tempus->user_level>1){ 
           ...
           //=================
           if($orgt->org_level==2){
                $org=Organization::orderBy('org_id', 'DESC')->where('org_id',$orgt->org_idParent)->get();
           }
           else{
                $org=Organization::orderBy('org_id', 'DESC')->where('org_id',$tempus->user_IdOrg)->get();
           }

        }
        else
        $org =Organization::where('org_level',1)->orderBy('org_id', 'DESC')->paginate(Setting::getconfig('config_showeverypage'));
    ...
    public function slideresult1($orgid){
        ...
        */
        ...
        $fans = $lava->DataTable();
        $value=array();
        if($this->dtchartda!=null && count($this->dtchartda)>0)
        ...
        */
        ...
        if($tempus->user_level==1){}
         ...
         //  $topic = Topic::where('topic_idorg',0)->orwhere('topic_idorg',$org->org_id)->get();
           ...
           $topic=Topic::where('topic_id',$org->org_topic_id)->get();
    ...
    */
    ...
    public function selectorg($parentorg_id=0)
    {
        ...
        if($orgt==null)$orgt=Organization::find(Setting::getconfig('config_orgtosurvey'));
        ...
        //-----lấy topi
        $topic=array();
        if($org!=null  && $org->org_topic_id>0)
        $topic = Topic::find($org->org_topic_id);
        //-----lấy ds nhân viên để đánh giá nếu đánh giá nhân viên và cấu hình chọn nhân viên đánh giá
        // loại 2 khảo sát nhân viên, phải chọn nhân viên
        if ($topic!=null  && ($topic->topic_type==2 && $org->org_isSelectEmp==1)){
            ...
            ->where('ks_organization.org_isActived', '=',1) 
            ->where(function($q) {
                        $q->where([
            ...
            return view('admin.survey.selectorg',['data'=>$data,'topic'=>$topic,'org'=>$child_org,'emp'=>$emp]);
            else return view('admin.survey.ngoaigiolamviec');
        }
        else
            if($topic!=null )
