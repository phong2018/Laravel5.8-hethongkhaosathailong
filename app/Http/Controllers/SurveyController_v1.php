...
class SurveyController extends Controller
{
    ...
    public function layketquabaocaongay($orgid){
        $this->xuatexcel=0;
        ...
        if(isset($_GET['filter_survey_topic_id']) && $_GET['filter_survey_topic_id']!=0){
            ...
            if(isset($_GET['traketqua'])){
                ...
                $datatrakq=array(
                    'header'=>$arr_header_excel,
                    'body'=>$arr_body,
                    'question'=>$ques,
                    'tentc'=>$organization['org_name'],
                );
                return $datatrakq;
        ...
        */
        ...
        return view('admin.survey.baocao1',['data'=>$data,'arr_body'=>$arr_body,'arr_header'=>$arr_header,'thongke'=>$thongke,'survey'=>$survey,'topic'=>$topic,'lava'=>$lava ]);
    }
    ...
    public function baocao1(){
      /*Lặp:
        ...
        foreach($orglv1 as $val){
            //$data['baocao']=
            $kqks=$this->layketquabaocaongay($val['org_id']);
            //print_r($kqks['header']);

            //print_r($kqks['body']);
            //return view('admin.survey.baocao1',['data'=>$kqks]);
            //print_r($kqks['question']);
            //break;
            //echo '---------------<br>';
            //break;

            //echo $val->org_name.'<br>';
            //echo '---------<br>';
            $users_quantri=DB::table('users')
                   ->join('ks_organization','users.user_IdOrg','=','ks_organization.org_id')  
                   ->select('users.*')
                   ->where('users.ID_Role','=',2)
                   ->where('isActived','=',1)
                   ->where(function($q)  use ($val) {
                        $q->where('ks_organization.org_id','=',$val['org_id'])
                        ->orwhere('ks_organization.org_idParent','=',$val['org_id']);
                   })
                   ->get();
            /*
            foreach($users_quantri as $val1){
                echo $val1->fullname.'--'.$val1->email.'<br>';
            }
            */

           // $data['to_email']="crm.nkh@gmail.com";
           
            $data = array(
              'to_email'=>"crm.nkh@gmail.com",
              'to_name'=>"Tên Người Nhận",
              'subject'=>'Báo Cáo Kết quả khảo sát ngày '. date("d-m-Y"),//Tựa đề của Email
            ...
            $data['tentc']=$kqks['tentc'];
            $data['header']=$kqks['header'];
            $data['body']=$kqks['body']; 
            $data['question']=$kqks['question']; 
